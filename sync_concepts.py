...
HASH_FILE = ROOT / ".concept-hashes.json"
...
def git_changed_concepts() -> Optional[list[Path]]:
    ...
    try:
        result = subprocess.run(
            ["git", "diff", "--name-only", "HEAD", "--", "Concepts/"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            timeout=10,
        ...
        result2 = subprocess.run(
            ["git", "diff", "--cached", "--name-only", "--", "Concepts/"],
            cwd=ROOT,
            capture_output=True,
            text=True,
            timeout=10,
        ...
        if result2.returncode == 0:
            staged = [ROOT / p.strip() for p in result2.stdout.splitlines() if p.strip()]
...
def detect_changed_concepts(force: bool = False) -> list[Path]:
    ...
    for p in all_concepts:
        current = file_hash(p)
        if saved.get(str(p.relative_to(ROOT))) != current:
            changed.append(p)
...
def sync_concept_file(concept_path: Path, dry_run: bool) -> bool:
    ...
    print(f"  UID    : {uid}")
    print(f"  슬러그 : {slug}")
    print(f"  대상   : Conditions/{slug}/")

    if not condition_dir.exists() and not dry_run:
        condition_dir.mkdir(parents=True, exist_ok=True)
        print(f"  [CREATE DIR] Conditions/{slug}/")
