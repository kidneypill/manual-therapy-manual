...
REGISTRY_PATH = MEDICAL_DATA / "_schema" / "uid-registry.yaml"
...
def main() -> None:
    ...
    if output_path.exists():
        print(
            f"\n⚠️  파일이 이미 존재합니다: {output_path.relative_to(REPO_ROOT)}\n"
    ...
    print(f"  경로    : medicalData/Concepts/{filename}")
    print(f"  UID     : {uid}")
    ...
    print(f"   1. medicalData/_schema/uid-registry.yaml > conditions 에 UID 등록:")
    print(f"      uid: \"{uid}\"")
    print(f"      name_en: \"{title}\"")
    print(f"      name_ko: \"(한글명 입력)\"")
    print(f"      file: \"medicalData/Conditions/{slug.lower()}-{seq}/_meta.yaml\"")
    ...
    print(f"\n[OK] 생성 완료: {output_path.relative_to(REPO_ROOT)}")
